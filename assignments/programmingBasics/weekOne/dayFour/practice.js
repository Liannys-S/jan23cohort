...
// Create a function that finds the min, max, sum, and average of an array and puts those results into a new array.  
//Takes any negative numbers and puts them in a new array, Then the positive odd numbers into an array and the positive even numbers into another array. 
// Print each array

function negPosOddEven(arr){
    var allResultsArr = betterNum(arr)
    var negNum = []
    var posEven = []
    var posOdd = []
    for(let i = 0; i < allResultsArr.length; i++){
        if(allResultsArr[i] < 0){
            negNum.push(allResultsArr[i])
        }
        else if(allResultsArr[i] > 0 && allResultsArr[i] % 2 === 0){
            posEven.push(allResultsArr[i])
        } 
        else if(allResultsArr[i] > 0 && allResultsArr[i] % 2 === 1){
            posOdd.push(allResultsArr[i])
        }
    }
    console.log('entire array: ', allResultsArr)
    console.log(`negative numbers: ${negNum} positive even: ${posEven} positive odd: ${posOdd}`)
